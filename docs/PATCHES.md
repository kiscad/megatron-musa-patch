...
| `torch.cuda.compat-layer` | `torch.cuda` | no CUDA bindings on the MUSA build; torchada + five overrides: live `is_available()` probe, `Tensor.type()` CUDA names, `CUDAGraph` alias, `.musa()` transfer fix for tensor subclasses (e.g. TE `Float8Tensor`), and the `torch._C._cuda_attach_out_of_memory_observer` binding (torch_musa's `_musa_attach_out_of_memory_observer`, same callback; Megatron-Bridge's OOM snapshot) |
