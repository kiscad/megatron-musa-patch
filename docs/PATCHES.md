...
| `torch.cuda.compat-layer` | `torch.cuda` | no CUDA bindings on the MUSA build; torchada + six overrides: live `is_available()` probe, `Tensor.type()` CUDA names, `CUDAGraph` alias, `.musa()` transfer fix for tensor subclasses (e.g. TE `Float8Tensor`), and the `torch._C._cuda_attach_out_of_memory_observer` binding (torch_musa's `_musa_attach_out_of_memory_observer`, same callback; Megatron-Bridge's OOM snapshot), and PyTorch's standard fp32-matmul TF32 switch (`torch.backends.cuda.matmul.allow_tf32` / `set_float32_matmul_precision`, default off as on CUDA) mirrored into torch_musa's own flag, which ignores both and defaults on (Qwen3-VL RoPE angles off by 0.35 rad at 1024 positions; muDNN's one flag also covers fp32 convolutions, so those become stricter than CUDA's TF32 default) |
...
| `megatron.mimo-colocated-communicator.subgroups-backend` | `models.mimo.comm.colocated_communicator:dist` | the same c10d `new_group` bypass from a third module global (`backend='nccl'` at colocated_communicator.py:92 and :104); the same proxy, `nccl` only |
...
| `torch.sort.bool-keys` | `torch.argsort` / `Tensor.argsort` / `torch.sort` / `Tensor.sort` | MUDNN's radix sort has no boolean kernel: every `torch.bool` input fails with `SortCall MUDNN failed` regardless of `stable`, `descending`, rank or length, while uint8/int8/int32/int64/fp16/bf16/fp32 all pass (measured matrix). MoE token permutation sorts its boolean routing map, so every MoE layer reaching the reference permutation died. Sort a uint8 view for exactly those inputs: `False<True` maps onto `0<1`, so the permutation and tie handling are identical |
