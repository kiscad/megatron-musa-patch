...
| `torch.cuda.compat-layer` | `torch.cuda` | MUSA 版没有 CUDA 绑定；torchada + 6 个覆盖：`is_available()` 实时探测、`Tensor.type()` CUDA 名字、`CUDAGraph` 别名、tensor 子类（如 TE `Float8Tensor`）的 `.musa()` 迁移修复、`torch._C._cuda_attach_out_of_memory_observer` 绑定（torch_musa 的 `_musa_attach_out_of_memory_observer`，回调签名一致；供 Megatron-Bridge 的 OOM 快照使用）、把 PyTorch 标准的 fp32 matmul TF32 开关（`torch.backends.cuda.matmul.allow_tf32` / `set_float32_matmul_precision`，默认关闭，与 CUDA 一致）同步到 torch_musa 自己的开关（它忽略这两个标准开关且默认开启；Qwen3-VL RoPE 在 1024 位置角度偏差 0.35 rad；muDNN 只有一个开关，同时管 fp32 卷积，因此卷积比 CUDA 默认的 TF32 更严格） |
...
| `torch.sort.bool-keys` | `torch.argsort` / `Tensor.argsort` / `torch.sort` / `Tensor.sort` | MUDNN 的 radix sort 没有 bool kernel：`torch.bool` 输入一律 `SortCall MUDNN failed`，与 `stable`、`descending`、形状无关，而 uint8/int8/int32/int64/fp16/bf16/fp32 全部正常（实测矩阵）。MoE token permutation 要对布尔 routing_map 排序，导致所有走参考置换的 MoE 层失败。只对这类输入在 uint8 视图上排序：`False<True` 对应 `0<1`，置换与并列处理完全一致 |
