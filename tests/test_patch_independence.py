...
def test_local_norm_flags_follow_class_in_any_registration_order(engine, stub_module, order):
    ...
    patches = [p for p in _layer_norm.PATCHES if p.id in {
    ...
    module = stub_module("megatron.core.fusions.fused_layer_norm", FusedLayerNorm=original,
                         HAVE_FUSED_LAYER_NORM=False, HAVE_PERSIST_LAYER_NORM=True)
...
def test_block_norm_works_without_local_class_replacement(engine, stub_module):
    ...
    engine.install()
    config = SimpleNamespace(normalization="RMSNorm", layernorm_zero_centered_gamma=False,
...
@pytest.mark.parametrize("order", list(permutations(range(3))))
def test_rope_dispatch_observes_live_kernels_without_mutating_config(engine, stub_module, monkeypatch, order):
    ...
    module = stub_module(_rope._ROPE_UTILS, fused_apply_rotary_pos_emb=None,
                         fused_apply_rotary_pos_emb_thd=None, apply_rotary_pos_emb=dispatch)
...
def test_native_rope_does_not_probe_apex(engine, stub_module, monkeypatch):
    ...
    kernel = lambda *args: None
    stub_module(_rope._ROPE_UTILS, fused_apply_rotary_pos_emb=kernel,
                fused_apply_rotary_pos_emb_thd=kernel, apply_rotary_pos_emb=kernel)
...
def test_training_wrappers_are_selectable_and_commute(engine, stub_module, order):
    ...
    engine.register([patches[i] for i in order])
    engine.install()
    args = SimpleNamespace(profile=True, use_pytorch_profiler=False,
                           overlap_grad_reduce=True, rank=1)
