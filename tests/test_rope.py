...
from megatron_musa_patch.patches import _rope
...
def test_apex_pair_is_installed_where_upstream_has_none(engine, stub_module):
    ...
    assert [record["status"] for record in engine.report()] == ["applied"] * 3
...
def test_transformer_engine_kernels_are_never_replaced(engine, stub_module):
    ...
    assert [record["status"] for record in engine.report()] == ["skipped", "skipped", "applied"]
...
def test_rope_fusion_switch_declines(stub_module, monkeypatch):
    _stub_apex(stub_module)
    assert _rope._apex_kernels() is not None

    monkeypatch.setenv("MEGATRON_MUSA_PATCH_ROPE_FUSION", "0")
    assert _rope._apex_kernels() is None
