...
statuses = {record["id"]: record["status"] for record in megatron_musa_patch.report()}
assert statuses["megatron.ssm.gated-delta-rule.tilelang"] == "applied", statuses
assert getattr(core_gdn.chunk_gated_delta_rule, "_megatron_musa_patch_tk_gdn", False), statuses
...
if bridge_status == "applied":
    ...
    assert getattr(bridge_gdn.chunk_gated_delta_rule, "_megatron_musa_patch_tk_gdn", False)
...
# GDN_TILELANG=0 in a fresh process restores FLA's binding (switch semantics
# are read at activation time, so this is a new interpreter).
env = dict(os.environ)
env["MEGATRON_MUSA_PATCH"] = "1"
env["MEGATRON_MUSA_PATCH_GDN_TILELANG"] = "0"
probe = subprocess.run(
    [
        sys.executable,
        "-c",
        "import megatron_musa_patch as m; m.apply();"
        "from megatron.core.ssm import gated_delta_net as g;"
        "st = {r['id']: r['status'] for r in m.report()};"
        "assert st['megatron.ssm.gated-delta-rule.tilelang'] == 'skipped', st;"
        "assert not getattr(g.chunk_gated_delta_rule, '_megatron_musa_patch_tk_gdn', False);"
