...
@pytest.mark.parametrize('installed,bound,blocked', [
    ('2.0', '==2.0.0', False), ('2.0.0', '<=2.0', False),
    ('2.0.0.1', '>2.0.0', False), ('2.0.0+vendor', '==2.0', False),
    ('2.0rc1', '>=2.0', False), ('unknown', '<2.1', True),
    ('2..0', '<3', True), ('2.0.post1', '==2', False),
...
@pytest.mark.parametrize('spec', ['vendor >=2junk', 'vendor <2.*', 'vendor ==2.0rc1',
                                  'vendor >=2..0', 'vendor >=2,', 'vendor ~=2.0'])
...
def test_gate_validation_shared(patch_type):
    args = dict(target='example:value', replace=lambda x: x) if patch_type is AttrPatch else dict(
        trigger='example', run=lambda: None)
    with pytest.raises(ValueError):
...
def test_blocked_gate_does_not_resolve_removed_target(engine, stub_module, monkeypatch):
    ...
    engine.register([AttrPatch('removed', 'gate_removed:OldClass.method',
                              lambda x: pytest.fail('must not run'), version_gates=('vendor <3',))])
    engine.install()
...
def test_source_probe_handles_top_level_module(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(_compat, 'find_spec_without_watchers',
...
def test_hook_gate_rechecked_on_reinstall(engine, stub_module, monkeypatch):
    ...
    calls = []
    engine.register([HookPatch("hook", "gate_hook", lambda: calls.append("run"),
                              undo=lambda: calls.append("undo"), version_gates=("vendor <3",))])
    engine.install()
    assert calls == []
    engine.unapply()
    monkeypatch.setenv("MEGATRON_MUSA_PATCH_IGNORE_VERSION_GATES", "1")
...
def test_hook_metadata_failure_is_reported(engine, stub_module, monkeypatch):
    ...
    engine.register([HookPatch("broken", "gate_error", lambda: pytest.fail("must not run"),
