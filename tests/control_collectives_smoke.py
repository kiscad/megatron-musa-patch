"""Two-rank validation of migrated patches against pristine upstream source.
...
from megatron.training import training, checkpointing
...
try:
    ...
    namespace = dict(__name__=training.__name__, torch=training.torch,
                     _TRAIN_START_TIME=timestamp + rank,
                     _LEGACY_TRAIN_START_TIME=timestamp + rank,
                     _STARTUP_TIMESTAMPS={'program_start': timestamp + rank})
    namespace['set_startup_timestamps'] = lambda **kw: namespace['_STARTUP_TIMESTAMPS'].update(kw)
    exec(compile(ast.fix_missing_locations(ast.Module(body=[function], type_ignores=[])),
                 '<upstream startup reductions>', 'exec'), namespace)
    namespace['pretrain']()
    ...
    exec('def save_checkpoint():\n    disk_io()\n    torch.distributed.barrier()\n    torch.distributed.barrier()', scope)
    from megatron_musa_patch.patches._control_collectives import _checkpoint_save
