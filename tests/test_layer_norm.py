...
@pytest.mark.parametrize("switch", ["upstream", "TE", " keep "])
def test_block_norm_upstream_opt_out(monkeypatch, switch):
    monkeypatch.setenv("MEGATRON_MUSA_PATCH_BLOCK_LAYERNORM", switch)
    assert _layer_norm._block_layer_norm_impl(object()) is None
