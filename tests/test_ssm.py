...
from megatron_musa_patch.patches import _ssm
...
def test_unknown_keyword_stays_with_fla():
    assert _ssm._normalized_call(("q", "k", "v", "g", "beta"), {"use_beta_sigmoid_in_kernel": True}) is None


def test_positional_and_keyword_forms_normalize():
    assert _ssm._normalized_call(("q", "k", "v"), {"g": "g", "beta": "b", "scale": 0.5}) == {
        "q": "q",
        "k": "k",
        "v": "v",
        "g": "g",
    ...
    assert _ssm._normalized_call(("q", "k", "v", "g", "beta", "extra"), {}) is None
    assert _ssm._normalized_call(("q", "k", "v"), {"g": "g"}) is None  # beta missing
    assert _ssm._normalized_call((), {"q": "q"}) is None  # fla requires all five


def test_duplicate_binding_stays_with_fla():
    assert _ssm._normalized_call(("q", "k"), {"k": "k2", "v": "v", "g": "g", "beta": "b"}) is None
...
def test_supported_call_dispatches_to_tilelang(engine, stub_module):
    ...
    assert getattr(core.chunk_gated_delta_rule, _ssm._MARKER, False)
...
def test_gdn_tilelang_off_declines_both_patches(engine, stub_module, monkeypatch):
    core, bridge, core_original, bridge_original, _ = install(engine, stub_module)
    monkeypatch.setenv("MEGATRON_MUSA_PATCH_GDN_TILELANG", "0")
    engine.register(_ssm.PATCHES)
    engine.install()

    assert core.chunk_gated_delta_rule is core_original
    assert bridge.chunk_gated_delta_rule is bridge_original
    assert all(record["status"] == "skipped" for record in engine.report())
...
def test_patches_are_independently_selectable(engine, stub_module, monkeypatch):
    core, bridge, core_original, bridge_original, _ = install(engine, stub_module)
    monkeypatch.setenv("MEGATRON_MUSA_PATCH_ONLY", CORE_ID)
    engine.register(_ssm.PATCHES)
    engine.install()

    assert core.chunk_gated_delta_rule is not core_original
    assert bridge.chunk_gated_delta_rule is bridge_original
    statuses = {record["id"]: record["status"] for record in engine.report()}
...
def test_reapply_does_not_stack_wrappers(engine, stub_module):
    ...
    engine.unapply()
    assert core.chunk_gated_delta_rule is core_original
    assert getattr(core_original, _ssm._MARKER, False) is False
