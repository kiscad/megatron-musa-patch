...
from megatron_musa_patch.patches import _control_collectives as control
...
def make_torch(backend='mccl'):
    dist = SimpleNamespace(
        is_initialized=lambda: True, get_backend=lambda: backend,
        group=SimpleNamespace(WORLD=object()), new_group=Mock(side_effect=[object(), object()]),
        all_reduce=Mock(), barrier=Mock(), ReduceOp=torch.distributed.ReduceOp,
    )
    return SimpleNamespace(distributed=dist, float64=torch.float64, int64=torch.int64,
                           tensor=torch.tensor), dist
...
def test_startup_integer_microseconds_preserves_upstream_globals():
    ...
    namespace = define('megatron.training.training', '''
...
@pytest.mark.parametrize('backend,name,dtype,op,async_op', [
...
def test_unrelated_reductions_pass_through(backend, name, dtype, op, async_op):
    ...
    proxy = control._startup_torch(base)
    namespace = define('megatron.training.training', f'''
def {name}(value):
    return torch.distributed.all_reduce(value, op=op, async_op=asynchronous)
''', torch=proxy, op=getattr(dist.ReduceOp, op), asynchronous=async_op)
...
def test_checkpoint_group_created_before_io_and_context_reset(stub_module, fail):
    ...
    def disk_io():
        assert dist.new_group.call_count == 1
    namespace = define('megatron.training.checkpointing', '''
def other_function():
...
def save_checkpoint():
    ...
    return 42
''', torch=proxy, subgroup=subgroup, disk_io=disk_io, fail=fail)
    module = stub_module('megatron.training.checkpointing', torch=proxy,
...
def test_checkpoint_companions_are_ordered_and_selectable(engine, stub_module, order):
    base, dist = make_torch()
    module = stub_module('megatron.training.checkpointing', torch=base,
...
@pytest.mark.parametrize('iteration,elapsed,saves,exits', [
    (1, 34, 0, False), (1000, 100, 1, False), (1, 236 * 60, 1, True),
    (1000, 236 * 60, 1, True),
])
def test_checkpoint_exit_policy(iteration, elapsed, saves, exits):
    ...
    try:
        function = next(n for n in tree.body if isinstance(n, ast.FunctionDef)
                        and n.name == 'checkpoint_and_decide_exit')
    except StopIteration:
        pytest.skip('checkpoint_and_decide_exit not found in upstream training.py')
    args = SimpleNamespace(exit_signal_handler=False, save='/tmp/checkpoint',
                           save_interval=1000, non_persistent_save_interval=None,
                           exit_duration_in_mins=235, exit_interval=None,
                           phase_transition_iterations=None)
    save = Mock()
    namespace = dict(get_args=lambda: args, get_timers=lambda: None,
                     time=SimpleNamespace(time=lambda: 1789452300 + elapsed),
                     _TRAIN_START_TIME=1789452300, save_checkpoint_and_time=save,
                     print_datetime=Mock(), torch=SimpleNamespace(
                         int=torch.int,
    ...
    exec(compile(ast.Module(body=[function], type_ignores=[]), str(TRAINING / 'training.py'), 'exec'), namespace)
    assert namespace['checkpoint_and_decide_exit'](None, None, None, iteration, 0, None, None) is exits
...
def test_two_rank_control_collectives():
    ...
    if os.environ.get('MEGATRON_MUSA_RUN_INTEGRATION') != '1':
        pytest.skip('set MEGATRON_MUSA_RUN_INTEGRATION=1 for two-rank MUSA control collectives')
