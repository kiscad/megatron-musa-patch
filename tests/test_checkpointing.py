...
def writer(stub_module, monkeypatch):
    monkeypatch.delenv("MEGATRON_MUSA_PATCH_CKPT_FORK", raising=False)
...
def test_fork_opt_out_leaves_upstream_unchanged(monkeypatch):
    monkeypatch.setenv("MEGATRON_MUSA_PATCH_CKPT_FORK", "1")
    assert _checkpointing._serial_writer(object()) is None
