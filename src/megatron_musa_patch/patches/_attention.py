...
_MATE_TRIED = False
_MATE_FN = None
#: Whether the installed mate build can serve a deterministic request.
_MATE_DETERMINISTIC: bool | None = None
_MATE_DETERMINISTIC_WARNED = False
...
def _mate_serves_deterministic() -> bool:
    """Source probe: can the installed mate honour ``deterministic=True``?

    mate 0.1.2 accepts the keyword and then asserts it is ``False``
    ("deterministic parameter is not supported yet"), so forwarding a
    deterministic request aborts the call. Probe the installed source instead
    of catching the assertion mid-forward, and treat an unreadable source as
    unsupported: declining costs the fast path, guessing costs the contract.
    """
    global _MATE_DETERMINISTIC
    if _MATE_DETERMINISTIC is None:
        fn = _mate_flash_fn()
        _MATE_DETERMINISTIC = False
        if fn is not None:
            import inspect
            ...
            try:
                ...
                source = "assert deterministic is False"
            _MATE_DETERMINISTIC = "assert deterministic is False" not in source
    return _MATE_DETERMINISTIC
...
def _mate_dense_forward(self, query, key, value, mask_name, layout):
    ...
    mate is called through its varlen entry point with the trivial uniform
    ``cu_seqlens`` (its dense form has no backward on 0.2.x); Megatron's sbhd
    layout is transposed and flattened on the way in, and the output is folded
    back and flattened to TE's dense contract (``[seq, batch, heads * v_dim]`` for
    ...
    if fn is None:
        return None
    if bool(getattr(self, "deterministic", False)) and not _mate_serves_deterministic():
        # The module asked for determinism and this mate build refuses it.
        # Decline so the caller reaches TE's unfused backend, which honours the
        # request; silently reducing it to deterministic=False would change the
        # contract the caller set.
        global _MATE_DETERMINISTIC_WARNED
        if not _MATE_DETERMINISTIC_WARNED:
            _MATE_DETERMINISTIC_WARNED = True
            _compat.logger.info(
                "the installed mate does not serve deterministic attention; those "
                "calls run TE's unfused backend instead"
            )
        return None
    import torch
    ...
    # mate's dense entry point has no backward on 0.2.x ("DNN fallback backward
    # currently supports varlen only"), so always use the varlen convention.
    # These calls are uniform-length, so flattening to (batch*seq, heads, dim)
    # with the trivial cu_seqlens is the same per-sequence computation, and it is
    # the only form whose gradients this vendor build produces.
    cu_seqlens = torch.arange(0, (batch + 1) * seq, seq, device=q.device, dtype=torch.int32)
    flat = [t.reshape(batch * seq, t.shape[2], t.shape[3]) for t in (q, k, v)]
    out = fn(
        *flat,
    ...
    return out if layout == "bshd" else out.transpose(0, 1)
