...
class _DistributedProxy:
    ...
    def all_reduce(self, tensor, *args, **kwargs):
        ...
        startup = (self._module_name == 'megatron.training.training'
                   and caller.f_globals.get('__name__') == self._module_name
                   and caller.f_code.co_name == 'pretrain')
        if (startup and op == dist.ReduceOp.MIN and group is None and not asynchronous
                and tensor.dtype == torch.float64 and tensor.numel() == 1
                and dist.is_initialized() and 'mccl' in str(dist.get_backend()).lower()):
            ...
            value = torch.tensor([int(tensor.item() * 1_000_000)],
    ...
    def barrier(self, *args, **kwargs):
        ...
        saving = (self._module_name == 'megatron.training.checkpointing'
                  and caller.f_globals.get('__name__') == self._module_name
...
PATCHES = (
    AttrPatch(
        ...
        target='megatron.training.training:torch', replace=_startup_torch,
        rationale='MCCL 2.11.4 reduced startup float64 MIN timestamps to 1.0.',
        strategy=('Use a module-local torch proxy only for synchronous scalar float64 MIN '
        ...
        remove_when=('Remove when MCCL float64 MIN preserves Unix timestamps and multi-rank '
                     'startup timestamps and duration-based exit tests pass.'),
    ),
    AttrPatch(
        id='megatron.training.checkpoint.host-barrier-proxy',
        target='megatron.training.checkpointing:torch', replace=_checkpoint_torch,
        rationale='MCCL device barriers can exceed the device watchdog during slow checkpoint I/O.',
        strategy=('Route only default barriers directly inside save_checkpoint through its '
        ...
        remove_when=('Remove with host-barrier-context when slow multi-rank checkpoint saves '
